...
3. 预览验证：`python -m http.server 8098`（Mac 用 `python3`）→ http://localhost:8098 ，双击桌面「扫雷」能开窗即正常（没装 Python 的机器用 `npx -y http-server -p 8098 -s`，等价）。
...
| Windows | `C:/Kimi Code/win98` | SSH 密钥（私钥在仓库 `.git/ssh/`，公钥名 `win98-relay-windows`），走 ssh.github.com:443，`core.sshCommand` 已配好 | `python -m http.server 8098`（Python 3.12.9 + Pillow 已装；**无 `python3` 命令**，那是商店 stub） | Playwright Chromium 已装；github.com 直连不稳，克隆备用镜像 `https://gh-proxy.com/https://github.com/viola0723/win98.git`；ffmpeg 与 inspira-ui 已就位（`../tools/`，gh-proxy 可作拉取 github 公开资源的通用备用通道） |
...
| 本机音视频处理（转码/压缩/探测） | `cd ../tools/ffmpeg && npm i ffmpeg-static`（二进制 `../tools/ffmpeg/node_modules/ffmpeg-static/ffmpeg.exe`；长期资产，勿当临时产物清理） |
